...
function data_graphic() {
	...
	moz.defaults.histogram = {
		rollover_callback: function (d, i) {
			$('#histogram svg .active_datapoint').html('Frequency Count: ' + d.y);
		},
		binned: false,
		bins: null,
...
function chart_title(args) {
	...
	if (args.target && args.title) {
		var newTitle;
		var optional_question_mark = (args.description) ? '<i class="fa fa-question-circle fa-inverse"></i>' : '';
...
function y_rug(args) {
	...
	var rug = svg.selectAll('line.y_rug').data(all_data).enter().append('svg:line').attr('x1', args.left + 1).attr('x2', args.left + buffer_size).attr('y1', args.scalefns.yf).attr('y2', args.scalefns.yf).attr('class', 'y-rug').attr('opacity', 0.3);
...
function y_axis(args) {
	...
	if (args.y_scale_type == 'log') {
		...
		args.scales.Y = d3.scale.log().domain([min_y, max_y]).range([args.height - args.bottom - args.buffer, args.top]).clamp(true);
	} else {
		args.scales.Y = d3.scale.linear().domain([min_y, max_y]).range([args.height - args.bottom - args.buffer, args.top]);
	}
	args.scales.Y_axis = d3.scale.linear().domain([min_y, max_y]).range([args.height - args.bottom - args.buffer, args.top]);
	var yax_format;
	...
	g = svg.append('g').classed('y-axis', true).classed('y-axis-small', args.use_small_class);
	if (args.y_label) {
	...
	if (!args.x_extended_ticks && !args.y_extended_ticks) {
		g.append('line').attr('x1', args.left).attr('x2', args.left).attr('y1', args.scales.Y(scale_ticks[0]).toFixed(2)).attr('y2', args.scales.Y(scale_ticks[last_i]).toFixed(2));
	}
	g.selectAll('.yax-ticks').data(scale_ticks).enter().append('line').classed('extended-y-ticks', args.y_extended_ticks).attr('x1', args.left).attr('x2', function () {
	...
	g.selectAll('.yax-labels').data(scale_ticks).enter().append('text').attr('x', args.left - args.yax_tick_length * 3 / 2).attr('dx', -3).attr('y', function (d) {
		return args.scales.Y(d).toFixed(2);
	}).attr('dy', '.35em').attr('text-anchor', 'end').text(function (d, i) {
		var o = yax_format(d);
		return o;
	})
	if (args.y_rug) {
...
function y_axis_categorical(args) {
	...
	args.scales.Y = d3.scale.ordinal().domain(args.categorical_variables).rangeRoundBands([args.height - args.bottom - args.buffer, args.top], args.padding_percentage, args.outer_padding_percentage);
	args.scalefns.yf = function (di) {
		return args.scales.Y(di[args.y_accessor]);
	}
	var svg = d3.select($(args.target).find('svg').get(0));
	var $svg = $($(args.target).find('svg').get(0));
	$svg.find('.y-axis').remove();
	var g = svg.append('g').classed('y-axis', true).classed('y-axis-small', args.use_small_class);
	if (!args.y_axis)return this;
	g.selectAll('text').data(args.categorical_variables).enter().append('svg:text').attr('x', args.left).attr('y', function (d) {
...
function x_rug(args) {
	...
	var rug = svg.selectAll('line.x_rug').data(all_data).enter().append('svg:line').attr('x1', args.scalefns.xf).attr('x2', args.scalefns.xf).attr('y1', args.height - args.top + buffer_size).attr('y2', args.height - args.top).attr('class', 'x-rug').attr('opacity', 0.3);
	if (args.color_accessor) {
		rug.attr('stroke', args.scalefns.color);
	}
	else {
...
function x_axis(args) {
	...
	if (args.chart_type == 'line') {
		...
		min_x = d3.min(args.data[0], function (d) {
			return d[args.x_accessor]
		});
	}
	else if (args.chart_type == 'histogram') {
	...
	args.scales.X = (args.time_series) ? d3.time.scale() : d3.scale.linear();
	args.scales.X.domain([min_x, max_x]).range([args.left + args.buffer, args.width - args.right - args.buffer - additional_buffer]);
	$svg.find('.x-axis').remove();
	if (!args.x_axis)return this;
	g = svg.append('g').classed('x-axis', true).classed('x-axis-small', args.use_small_class);
	...
	g.selectAll('.xax-labels').data(args.scales.X.ticks(args.xax_count)).enter().append('text').attr('x', function (d) {
		return args.scales.X(d).toFixed(2);
	}).attr('y', (args.height - args.bottom + args.xax_tick_length * 7 / 3).toFixed(2)).attr('dy', '.50em').attr('text-anchor', 'middle').text(function (d) {
	...
	if (args.time_series && args.show_years) {
		...
		g = g.append('g').classed('year-marker', true).classed('year-marker-small', args.use_small_class);
		g.selectAll('.year_marker').data(years).enter().append('line').attr('x1', function (d) {
			return args.scales.X(d).toFixed(2);
		}).attr('x2', function (d) {
			return args.scales.X(d).toFixed(2);
		}).attr('y1', args.top).attr('y2', args.height - args.bottom);
		var yformat = d3.time.format('%Y');
		g.selectAll('.year_marker').data(years).enter().append('text').attr('x', function (d) {
			return args.scales.X(d).toFixed(2);
		}).attr('y', (args.height - args.bottom + args.xax_tick_length * 7 / 1.3).toFixed(2)).attr('dy', args.use_small_class ? -3 : 0).attr('text-anchor', 'middle').text(function (d) {
			return yformat(d);
		});
	}
	;
...
function init(args) {
	'use strict';
	var defaults = {target: null, title: null, description: null};
	var args = arguments[0];
	if (!args) {
		args = {};
	}
	...
	if ((svg.find('.main-line').length > 0 && args.chart_type != 'line') || (svg.find('.points').length > 0 && args.chart_type != 'point') || (svg.find('.histogram').length > 0 && args.chart_type != 'histogram') || (svg.find('.barplot').length > 0 && args.chart_type != 'bar')) {
		$(args.target).empty();
	}
	if ($.trim($(args.target).html()) == '') {
		d3.select(args.target).append('svg').classed('linked', args.linked).attr('width', svg_width).attr('height', svg_height);
	}
	...
	chart_title(args);
	args.use_small_class = args.height - args.top - args.bottom - args.buffer <= args.small_height_threshold && args.width - args.left - args.right - args.buffer * 2 <= args.small_width_threshold || args.small_text;
	if (args.data.length < $(args.target).find('svg .main-line').length) {
		if (args.custom_line_color_map.length > 0) {
			...
			var lines_to_remove = arrDiff(array_full_series(args.max_data_size), args.custom_line_color_map);
			for (var i = 0; i < lines_to_remove.length; i++) {
				$(args.target).find('svg .main-line.line' + lines_to_remove[i] + '-color').remove();
			}
		}
		else {
...
function markers(args) {
	...
	if (args.markers) {
		...
		gm = svg.append('g').attr('class', 'markers');
		gm.selectAll('.markers').data(args.markers.filter(function (d) {
			return (args.scales.X(d[args.x_accessor]) > args.buffer + args.left) && (args.scales.X(d[args.x_accessor]) < args.width - args.buffer - args.right);
		})).enter().append('line').attr('x1', function (d) {
			return args.scales.X(d[args.x_accessor]).toFixed(2);
		}).attr('x2', function (d) {
			return args.scales.X(d[args.x_accessor]).toFixed(2);
		}).attr('y1', args.top).attr('y2', function () {
		...
		}).attr('y', args.top - 8).attr('text-anchor', 'middle').text(function (d) {
			return d['label'];
		});
	}
	if (args.baselines) {
		svg.selectAll('.baselines').remove();
		gb = svg.append('g').attr('class', 'baselines');
		gb.selectAll('.baselines').data(args.baselines).enter().append('line').attr('x1', args.left + args.buffer).attr('x2', args.width - args.right - args.buffer).attr('y1', function (d) {
			return args.scales.Y(d['value']).toFixed(2);
		}).attr('y2', function (d) {
			return args.scales.Y(d['value']).toFixed(2);
		});
		gb.selectAll('.baselines').data(args.baselines).enter().append('text').attr('x', args.width - args.right - args.buffer).attr('y', function (d) {
			return args.scales.Y(d['value']).toFixed(2);
		}).attr('dy', -3).attr('text-anchor', 'end').text(function (d) {
			return d['label'];
		});
	}
	return this;
...
var button_layout = function (target) {
	...
	this.manual_button = function (feature, feature_set, callback) {
		...
		this.manual_callback[feature] = callback;
		return this;
	}
	...
	this.display = function () {
		...
		for (var feature in this.feature_set) {
			...
			$(this.target + ' div.segments').append('<div class="btn-group ' + this._strip_punctuation(feature) + '-btns text-left">' + '<button type="button" class="btn btn-default btn-lg dropdown-toggle" data-toggle="dropdown">' + "<span class='which-button'>" + (this.public_name.hasOwnProperty(feature) ? this.public_name[feature] : feature) + "</span>" + "<span class='title'>" + (this.manual_callback.hasOwnProperty(feature) ? this.feature_set[feature][0] : 'all') + "</span>" + '<span class="caret"></span>' + '</button>' + '<ul class="dropdown-menu" role="menu">' +
			...
			for (var i = 0; i < features.length; i++) {
				if (features[i] != 'all' && features[i] != undefined) {
					$(this.target + ' div.' + this._strip_punctuation(feature) + '-btns ul.dropdown-menu').append('<li><a href="#" data-feature="' + this._strip_punctuation(feature) + '" data-key="' + features[i] + '">'
					+ features[i] + '</a></li>');
				}
			}
...
charts.line = function (args) {
	...
	this.mainPlot = function () {
		...
		var g;
		var data_median = 0;
		var area = d3.svg.area().x(args.scalefns.xf).y0(args.scales.Y.range()[0]).y1(args.scalefns.yf).interpolate(args.interpolate);
		var confidence_area;
		if (args.show_confidence_band) {
			var confidence_area = d3.svg.area().x(args.scalefns.xf).y0(function (d) {
		...
		var line = d3.svg.line().x(args.scalefns.xf).y(args.scalefns.yf).interpolate(args.interpolate);
		var flat_line = d3.svg.line().x(args.scalefns.xf).y(function () {
		...
		for (var i = args.data.length - 1; i >= 0; i--) {
			...
			if (args.show_confidence_band) {
				svg.append('path').attr('class', 'confidence-band').attr('d', confidence_area(args.data[i]));
			}
			...
			if (args.area && !args.use_data_y_min && !args.y_axis_negative && args.data.length <= 1) {
				if ($area.length > 0) {
					d3.selectAll($(args.target).find('svg path.area' + (line_id) + '-color')).transition().duration(function () {
						return (args.transition_on_update) ? 1000 : 0;
					}).attr('d', area(args.data[i]));
				}
				else {
					svg.append('path').attr('class', 'main-area ' + 'area' + (line_id) + '-color').attr('d', area(args.data[i]));
				}
			} else if ($area.length > 0) {
				$area.remove();
			}
			if ($(args.target).find('svg path.line' + (line_id) + '-color').length > 0) {
				d3.selectAll($(args.target).find('svg path.line' + (line_id) + '-color')).transition().duration(function () {
					return (args.transition_on_update) ? 1000 : 0;
				...
				if (args.animate_on_load) {
					...
					svg.append('path').attr('class', 'main-line ' + 'line' + (line_id) + '-color').attr('d', line(args.data[i]));
				}
			}
			if (args.legend) {
	...
	this.rollover = function () {
		...
		svg.append('text').attr('class', 'active_datapoint').classed('active-datapoint-small', args.use_small_class).attr('xml:space', 'preserve').attr('x', args.width - args.right).attr('y', args.top / 2).attr('text-anchor', 'end');
		svg.append('circle').classed('line_rollover_circle', true).attr('cx', 0).attr('cy', 0).attr('r', 0);
		var line_id = 1;
		for (var i = 0; i < args.data.length; i++) {
			for (var j = 0; j < args.data[i].length; j++) {
				if (args.custom_line_color_map.length > 0) {
		...
		if (args.data.length > 1) {
			...
			var g = svg.append('g').attr('class', 'voronoi')
			...
			g.selectAll('path').data(voronoi(data_nested)).enter().append('path').attr("d", function (d) {
				return "M" + d.join("L") + "Z";
			}).datum(function (d) {
				return d.point;
			...
			}).on('mouseover', this.rolloverOn(args)).on('mouseout', this.rolloverOff(args));
		}
		else {
			var line_id = 1;
			if (args.custom_line_color_map.length > 0) {
			...
			g.selectAll('.rollover-rects').data(args.data[0]).enter().append('rect').attr('class', function (d, i) {
				if (args.linked) {
					var v = d[args.x_accessor];
					var formatter = d3.time.format('%Y-%m-%d');
					var id = (typeof v === 'number') ? i : formatter(v);
				...
				return (args.data.length > 1) ? args.scalefns.yf(d) - 6 : args.top;
				...
				return (args.data.length > 1) ? 12 : args.height - args.bottom - args.top - args.buffer;
			}).attr('opacity', 0).on('mouseover', this.rolloverOn(args)).on('mouseout', this.rolloverOff(args));
		}
		return this;
	}
	this.rolloverOn = function (args) {
		...
		return function (d, i) {
			svg.selectAll('circle.line_rollover_circle').attr('class', "").attr('class', 'area' + d['line_id'] + '-color').classed('line_rollover_circle', true).attr('cx', function () {
				return args.scales.X(d[args.x_accessor]).toFixed(2);
			}).attr('cy', function () {
				return args.scales.Y(d[args.y_accessor]).toFixed(2);
			...
			if (args.linked && !globals.link) {
				...
				var formatter = d3.time.format('%Y-%m-%d');
				var id = (typeof v === 'number') ? i : formatter(v);
				d3.selectAll('.line' + d['line_id'] + '-color.roll_' + id).each(function (d, i) {
			...
			var fmt = d3.time.format('%H:%M %e %b');
			if (args.format == 'count') {
				var num = function (d_) {
					var is_float = d_ % 1 != 0;
			...
			if (args.show_rollover_text) {
				svg.select('.active_datapoint').text(function () {
					if (args.time_series) {
	...
	this.rolloverOff = function (args) {
		...
		return function (d, i) {
			if (args.linked && globals.link) {
				...
				var formatter = d3.time.format('%Y-%m-%d');
				var id = (typeof v === 'number') ? i : formatter(v);
				d3.selectAll('.roll_' + id).each(function (d, i) {
...
charts.histogram = function (args) {
	...
	this.mainPlot = function () {
		...
		var g = svg.append("g").attr("class", "histogram");
		var bar = g.selectAll(".bar").data(args.data[0]).enter().append("g").attr("class", "bar").attr("transform", function (d) {
	...
	this.rollover = function () {
		...
		$svg.find('.transparent-rollover-rect').remove();
		$svg.find('.active_datapoint').remove();
		svg.append('text').attr('class', 'active_datapoint').attr('xml:space', 'preserve').attr('x', args.width - args.right).attr('y', args.top / 2).attr('text-anchor', 'end');
		var g = svg.append('g').attr('class', 'transparent-rollover-rect')
		var bar = g.selectAll(".bar").data(args.data[0]).enter().append("g").attr("class", "rollover-rects").attr("transform", function (d) {
			return "translate(" + (args.scales.X(d[args.x_accessor])) + "," + 0 + ")";
		});
		bar.append("rect").attr("x", 1).attr("y", 0).attr("width", function (d, i) {
	...
	this.rolloverOn = function (args) {
		...
		return function (d, i) {
			...
			d3.selectAll($(args.target).find(' svg .bar :eq(' + i + ')')).classed('active', true);
			if (args.show_rollover_text) {
				svg.select('.active_datapoint').text(function () {
					if (args.time_series) {
						var dd = new Date(+d[args.x_accessor]);
	...
	this.rolloverOff = function (args) {
		var svg = d3.select($(args.target).find('svg').get(0));
		return function (d, i) {
			d3.selectAll($(args.target).find('svg .bar :eq(' + i + ')')).classed('active', false);
			svg.select('.active_datapoint').text('');
		}
	}
	this.init(args);
...
charts.point = function (args) {
	...
	this.mainPlot = function () {
		...
		g = svg.append('g').classed('points', true);
		var pts = g.selectAll('circle').data(args.data[0]).enter().append('svg:circle').attr('class', function (d, i) {
			return 'path-' + i;
		}).attr('cx', args.scalefns.xf).attr('cy', args.scalefns.yf);
		if (args.color_accessor != null) {
	...
	this.rollover = function () {
		...
		$svg.find('.active_datapoint').remove();
		svg.append('text').attr('class', 'active_datapoint').attr('xml:space', 'preserve').attr('x', args.width - args.right).attr('y', args.top / 2).attr('text-anchor', 'end');
		var voronoi = d3.geom.voronoi().x(args.scalefns.xf).y(args.scalefns.yf).clipExtent([[args.buffer, args.buffer], [args.width - args.buffer, args.height - args.buffer]]);
		var paths = svg.append('g').attr('class', 'voronoi');
		paths.selectAll('path').data(voronoi(args.data[0])).enter().append('path').attr('d', function (d) {
			if (d == undefined)return;
			return 'M' + d.join(',') + 'Z';
		}).attr('class', function (d, i) {
			return 'path-' + i;
		}).style('fill-opacity', 0).on('mouseover', this.rolloverOn(args)).on('mouseout', this.rolloverOff(args));
		return this;
	}
	this.rolloverOn = function (args) {
		var svg = d3.select($(args.target).find('svg').get(0));
		return function (d, i) {
			...
			var pts = svg.selectAll('.points circle.path-' + i).classed('selected', true);
			...
			if (args.linked && !globals.link) {
				globals.link = true;
				d3.selectAll('.voronoi .path-' + i).each(function () {
					d3.select(this).on('mouseover')(d, i);
				})
			}
			var fmt = d3.time.format('%b %e, %Y');
	...
	this.rolloverOff = function (args) {
		...
		return function (d, i) {
			...
			var pts = svg.selectAll('.points circle').classed('unselected', false).classed('selected', false);
			if (args.size_accessor) {
				pts.attr('r', args.scalefns.size);
			}
			else {
...
charts.bar = function (args) {
	...
	this.mainPlot = function () {
		...
		var g = svg.append('g').classed('barplot', true);
		var appropriate_height = args.scales.Y.rangeBand() / 1.5;
		g.selectAll('.bar').data(data).enter().append('rect').classed('bar', true).attr('x', args.scales.X(0)).attr('y', function (d) {
			return args.scalefns.yf(d) + appropriate_height / 2;
		}).attr('height', appropriate_height).attr('width', function (d) {
			return args.scalefns.xf(d) - args.scales.X(0)
		});
		if (args.predictor_accessor) {
			var pp = args.predictor_proportion;
			var pp0 = pp - 1;
			g.selectAll('.prediction').data(data).enter().append("rect").attr('class', 'bar-prediction').attr('x', args.scales.X(0)).attr('y', function (d) {
				return args.scalefns.yf(d) + pp0 * appropriate_height / (pp * 2) + appropriate_height / 2;
			}).attr('height', appropriate_height / pp).attr('width', function (d) {
				return args.scales.X(d[args.predictor_accessor]) - args.scales.X(0);
			});
		}
		if (args.baseline_accessor) {
			g.selectAll('.baseline').data(data).enter().append("line").attr('class', 'bar-baseline').attr('x1', function (d) {
	...
	this.rollover = function () {
		...
		$svg.find('.transparent-rollover-rect').remove();
		$svg.find('.active_datapoint').remove();
		svg.append('text').attr('class', 'active_datapoint').attr('xml:space', 'preserve').attr('x', args.width - args.right).attr('y', args.top / 2).attr('dy', '.35em').attr('text-anchor', 'end');
		var g = svg.append('g').attr('class', 'transparent-rollover-rect')
	...
	this.rolloverOn = function (args) {
		...
		return function (d, i) {
			...
			d3.selectAll($(args.target + ' svg g.barplot .bar:eq(' + i + ')')).classed('active', true);
			if (args.show_rollover_text) {
				svg.select('.active_datapoint').text(function () {
					if (args.time_series) {
						var dd = new Date(+d[args.x_accessor]);
	...
	this.rolloverOff = function (args) {
		var svg = d3.select($(args.target).find('svg').get(0));
		return function (d, i) {
			d3.selectAll($(args.target).find('svg g.barplot .bar:eq(' + i + ')')).classed('active', false);
			svg.select('.active_datapoint').text('');
		}
	}
	this.init(args);
...
charts.missing = function (args) {
	...
	this.init = function (args) {
		chart_title(args);
		d3.select(args.target).selectAll('svg').data([args]).enter().append('svg').attr('width', args.width).attr('height', args.height);
		d3.select(args.target).selectAll('svg *').remove()
		var svg = d3.select(args.target).select('svg')
		svg.classed('missing', true);
		if (args.legend_target)
			$(args.legend_target).html('');
		svg.append('rect').attr('class', 'missing-pane').attr('x', args.left).attr('y', args.top).attr('width', args.width - (args.left * 2)).attr('height', args.height - (args.top * 2));
		svg.selectAll('.missing_text').data([args.missing_text]).enter().append('text').attr('class', 'missing-text').attr('x', args.width / 2).attr('y', args.height / 2).attr('dy', '.50em').attr('text-anchor', 'middle').text(args.missing_text)
		return this;
	}
	this.init(args);
	return this;
...
function process_line(args) {
	'use strict';
	var is_time_series = ($.type(args.data[0][0][args.x_accessor]) == 'date') ? true : false;
...
function process_histogram(args) {
	...
	if (args.binned == false) {
		...
		args.processed_data = hist(extracted_data).map(function (d) {
...
function add_ls(args) {
	...
	svg.append('svg:line').attr('x1', args.scales.X(min_x)).attr('x2', args.scales.X(max_x)).attr('y1', args.scales.Y(args.ls_line.fit(min_x))).attr('y2', args.scales.Y(args.ls_line.fit(max_x))).attr('class', 'least-squares-line')
}
function add_lowess(args) {
	...
	svg.append('path').attr('d', line(lowess)).attr('class', 'lowess-line')
}
...
function least_squares(x_, y_) {
	...
	return {
		x0: x0, beta: beta, fit: function (x) {
			return x0 + x * beta;
		}
	}
}
...
function _weighted_means(wxy) {
	...
	return {
		...
		})) / wsum, ybar: d3.sum(wxy.map(function (wxyi) {
...
function _calculate_lowess_fit(x, y, alpha, inc, residuals) {
	...
	for (var i = 0; i < x_proto.length; i += 1) {
		...
		xi_neighbors = xi_neighbors.map(function (wxy) {
			return {w: _tricube_weight(wxy[0] / delta_i) * wxy[3], x: wxy[1], y: wxy[2]}
		})
